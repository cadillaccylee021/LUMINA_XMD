module.exports = [
  {
    ...
    ban: true,
    gcban: true,
    execute: async (m, { text, command, ednut, prefix, reply }) => {
      const [text1, text2] = text.split('|').map(v => v.trim());
      if (!text1 || !text2) return reply(`Please use the correct format. Example: ${prefix + command} lumina | md`);
      try {
...

  {
    ...
    ban: true,
    gcban: true,
    execute: async (m, { text, ednut, reply }) => {
      if (!text) return reply('Please provide a name. Example: warningsign lumina');
      try {
...

  {
    ...
    gcban: true,
    execute: async (m, { text, ednut, reply }) => {
      if (!text) return reply('Please provide a name or phrase. Example: pubg lumina');
      try {
...

  {
    ...
    gcban: true,
    execute: async (m, { text, ednut, reply }) => {
      if (!text.includes('|')) return reply('Please provide two words separated by "|". Example: avengers lumina|Md');
      try {
...

  {
    ...
    gcban: true,
    execute: async (m, { text, ednut, reply }) => {
      if (!text.includes('|')) return reply('Use format: graffiti text1|text2. Example: graffiti lumina|md');
      try {
...

  {
    ...
    gcban: true,
    execute: async (m, { text, ednut, reply }) => {
      if (!text) return reply('Please provide text. Example: matrix lumina');
      try {
  ...
  },
  {
    ...
    ban: true,
    gcban: true,
    execute: async (m, { text, ednut, sleep, fetch, reply }) => {
      ...
      try {
        ...
        if (!data?.result?.length) {
          return reply(`No images found for: *${text}* Try different keywords or use English.`);
        }
  ...
  },
  {
    ...
    gcban: true,
    execute: async (m, { text, ednut, reply }) => {
      if (!text) return reply('Please provide text. Example: onepiece lumina');
      try {
