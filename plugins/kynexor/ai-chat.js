...

{
  ...
  async execute(m, { args, reply }) {
    const query = args.join(" ").trim();
    if (!query) return reply("🤖 Please ask something like `.luminaai What is philosophy?`");
    ...
    try {
      ...
      if (!json?.status || !json?.result) {
        return reply("❌ lumina AI couldn't generate a response. Please try again.");
      }
      
      const response = `╭───『 LUMINA-XMD 』───⳹\n│\n│ ${json.result}\n│\n╰────────────────⳹`;
      await reply(response);
    } catch (e) {
      console.error("luminaAI Error:", e);
      reply("❌ An error occurred while contacting lumina AI.");
    }
  }
},
{
